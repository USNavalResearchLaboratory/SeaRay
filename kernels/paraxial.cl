...
{
	// TEMPORAL POINT PROTOCOL
	// chi = [t][x][y], effective susceptibility
	// E = [t][x][y], electric field envelope
	...
	chi[idx] += 0.75*ng[j0*Nk + k0]*chi3*e2;
}

__kernel void SourcePolarization(__global tw_Complex * P, __global tw_Float * chi, __global tw_Complex * E)
{
	// TEMPORAL POINT PROTOCOL
	// P = [t][x][y]
	const int i0 = get_global_id(0);
