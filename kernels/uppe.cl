...
// deprecated
__kernel void SetKerrPolarization(
	__global tw_Float * P,
	__global tw_Float * E,
	...
	const tw_Float chi3)
{
	...
	// P[t][x][y], <E.E> on input, polarization on output
	// E[t][x][y], real valued electric field
	// ng[x][y] = gas density in plasma units
	// chi3 = chi3 divided by reference density in plasma units
	const int i0 = get_global_id(0);
	const int j0 = get_global_id(1);
	const int k0 = get_global_id(2);
	const int Ni = get_global_size(0);
	...
	const int Nk = get_global_size(2);
	const int idx = i0*Nj*Nk + j0*Nk + k0;
	P[idx] *= 1.5*ng[j0*Nk + k0]*chi3*E[idx];
}

__kernel void AddKerrChi(
	__global tw_Float * chi,
	__global tw_Float * E,
	__global tw_Float * ng,
	const tw_Float chi3)
{
	// TEMPORAL POINT PROTOCOL
	// chi = [t][x][y], instantaneous susceptibility
	// E = [t][x][y], instantaneous electric field
	// ng = [x][y], gas density, plasma units
	// chi3 = chi3 divided by reference density in plasma units
	const int i0 = get_global_id(0);
	const int j0 = get_global_id(1);
	const int k0 = get_global_id(2);
	...
	const int Nk = get_global_size(2);
	const int idx = i0*Nj*Nk + j0*Nk + k0;
	chi[idx] += ng[j0*Nk + k0]*chi3*E[idx]*E[idx];
}

__kernel void SourcePolarization(__global tw_Float * P, __global tw_Float * chi, __global tw_Float * E)
{
	// TEMPORAL POINT PROTOCOL
	// P = [t][x][y]
	const int i0 = get_global_id(0);
