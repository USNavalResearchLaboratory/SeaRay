...
def DataFromGrid(w,x,y,wn,xn,yn,the_data):
	...
	pts[...,2] = y
	return obj(pts)
# TODO: need to interpolate this more robustly
def CylGridFromInterpolation(w,x,y,vals,wn=1,xn=100,yn=100,fill=0.0):
	...
	for i in range(wn.shape[0]):
		for j in range(yn.shape[0]):
			...
			if len(x[sel])==0:
				the_data[i,:,j] = 0
				#logging.critical("no rays found in (w,phi) bins, (w,phi) dimensions should be matched for all objects")
				#exit(1)
			else:
				idx = np.argmin(x[sel])
				v0 = vals[sel][idx]
...
def GridFromInterpolation(w,x,y,vals,wn=1,xn=100,yn=100,fill=0.0):
	...
	for i in range(wn.shape[0]):
		...
		sel = np.where(wcond)
		if len(x[sel])==0:
			logging.critical("no rays found in w-bin, w-dimension should be matched for all objects")
			exit(1)
		else:
...
class HankelTransformTool(TransverseModeTool):
	...
	def AllocateDeviceMemory(self,data_shape):
		...
		self.scratch_dev = pyopencl.array.empty(self.cl.q,data_shape,np.cdouble)
		self.cl.q.finish()
	def FreeDeviceMemory(self):
		self.H_dev = None
		self.L_dev = None
...
def WignerTransform(A,ds):
	N = A.shape[0]
	M = int(N/2) + 1
	corr = np.zeros((N,M)).astype(np.cdouble)
	Ai = np.zeros(N*2-1).astype(np.cdouble)
	Ai[::2] = A
