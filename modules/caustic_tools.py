...
class CausticTool:
	...
	def GetFields(self,dz,A):
		...
		phase_adv = np.ones(ans.shape).astype(np.cdouble) * z_nodes[np.newaxis,np.newaxis,np.newaxis,:]
...
def spherical_to_cylindrical(A,q_list,r_list,rho_pts,z_pts):
	...
	B = np.zeros((rho_pts,z_pts)).astype(np.cdouble)
...
def GetDivergence(Ax,Az,dr,dz,cl):
	ans = np.zeros(Ax.shape).astype(np.cdouble)
	Ax_dev = pyopencl.array.to_device(cl.queue,Ax)
	Az_dev = pyopencl.array.to_device(cl.queue,Az)
	ans_dev = pyopencl.array.to_device(cl.queue,ans)
	cl.queue.finish()
...
def GetLaplacian(A,dr,dz,m,cl):
	ans = np.zeros(A.shape).astype(np.cdouble)
	A_dev = pyopencl.array.to_device(cl.queue,A)
	ans_dev = pyopencl.array.to_device(cl.queue,ans)
	cl.queue.finish()
