'''
Module :samp:`photoionization_base`
====================================

The following is derived from ``twutils``, part of turboWAVE.
The ``ionization`` module uses this as a basis for preparing data for OpenCL ionization calculations.
...
class AtomicUnits:
	...
	def rate_to_mks(self,rate):
		'''Convert a rate from atomic to mks units'''
		return rate/self.ta
	...
	def rate_to_au(self,rate):
		'''Convert a rate from mks to atomic units'''
		return rate*self.ta
...
class Ionization(AtomicUnits):
	...
	def __init__(self,averaging,Uion,Z,lstar=0,l=0,m=0,w0=0,terms=1):
		...
		self.C_exp = 0.0
	def GetCoeff(self):
		return self.C_pre,self.C_pow,self.C_exp
	def ToAverage(self):
	...
	def ExtractEikonalForm(self,E,dt,w00=0.0,bandwidth=1.0):
		...
		return ExtractEikonalForm(E,dt,w00,bandwidth)
	def Rate(self,Es):
...
class StitchedPPT(PPT_Tunneling,PPT):
	...
	def Rate(self,Es):
		...
		rate[tunneling_selection] = PPT_Tunneling.Rate(self,Es[tunneling_selection])
		rate[mpi_selection] = PPT.Rate(self,Es[mpi_selection])
		return rate
