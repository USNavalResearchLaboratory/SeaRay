...
class attenuator(Vacuum):
	...
	def chi(self,w):
		...
		if terms>0:
			...
			opacity = np.zeros(w.shape+a.shape).astype(np.cdouble)
...
class sellmeier_medium_alt2(isotropic_medium):
	...
	def __init__(self,mks_length,Nr,Ar,lr,rel_linewidth):
		...
		if type(rel_linewidth)==np.single or type(rel_linewidth)==np.double:
	...
	def chi(self,w):
		...
		atten = 0#2j*np.einsum('...,k',np.ones(freqs),self.linewidth)
